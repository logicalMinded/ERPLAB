...
*   **⚠️考量**：頻繁的密碼雜湊運算易產生大量短生命週期的字串與陣列物件，增加 ***GC (垃圾回收) 壓力***；常規字串比對亦存在***時序攻擊 (Timing Attack)*** 的安全疑慮。
*   **🛠️實作**：密碼驗證模組採用 `PBKDF2` 演算法，並相容 `Identity V3` 的 Big-Endian 二進位封裝格式。處理過程中採用 `stackalloc` 與 `Span<byte>` 進行記憶體物理切片操作。
